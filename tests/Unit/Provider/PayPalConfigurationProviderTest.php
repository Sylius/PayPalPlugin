...
final class PayPalConfigurationProviderTest extends TestCase
{
    ...
    public function test_it_considers_apple_pay_disabled_by_default_when_the_config_key_is_absent(): void
    {
        $channel = $this->configurePayPalPaymentMethodConfig([]);

        self::assertFalse($this->payPalConfigurationProvider->isApplePayEnabled($channel));
    }

    public function test_it_considers_apple_pay_enabled_when_explicitly_set_to_true(): void
    {
        $channel = $this->configurePayPalPaymentMethodConfig(['apple_pay_enabled' => true]);

        self::assertTrue($this->payPalConfigurationProvider->isApplePayEnabled($channel));
    }
